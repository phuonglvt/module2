...
SET BITS=64
SET USER_JDK64_FILE=%USERPROFILE%\.PhpStorm2019.1\config\phpstorm%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%USERPROFILE%\.PhpStorm2019.1\config\phpstorm%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%USERPROFILE%\.PhpStorm2019.1\config\phpstorm%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_PHPSTORM_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_PHPSTORM.hprof" -Didea.paths.selector=PhpStorm2019.1 %IDE_PROPERTIES_PROPERTY%
